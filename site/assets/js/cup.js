...
(function () {
  ...
  /* وأنعم منها: smootherstep. الفرق أن smooth تُصفّر السرعة عند
     الطرفين لكن تسارعَها يقفز قفزة، فيُقرأ في الحركات الكبيرة
     الطويلة (تفكّك الرسمة، طلوع البحر، الذوبان) نتوءاً خفيفاً في
     بدايتها ونهايتها. وهذه تُصفّر السرعة والتسارع معاً. */
  ...
  function draw(p, tms) {
    ...
    /* والسقوط ينتهي عند التسليم بالضبط، لا يفيض بعده.
       كان يفيض 0.10 فيبقى الكوب عند التسليم في 80% من هبوطه، فتنقلب
       سرعته من سرعة الإصبع (نحو 410px/ث في تمريرٍ عادي) إلى سرعة
       الساعة (115px/ث): تباطؤٌ ثلاثة أضعاف ونصف في اللحظة التي
       المفروض فيها الجاذبيةُ تُسرِّع — وهو أظهر نتوء في المشهد كله.
       الآن d هي نفسها نسبةُ دخول الشريط، فيلمس الطقمُ الطاولةَ في
       اللحظة التي يملأ فيها الشريطُ الشاشة، ويتسلّم الزمنُ مشهداً
       ساكناً: لا سرعةَ تنقلب لأنه ما بقي شيء يتحرّك. */
    var d = clamp01(p / pin);
    ...
    /* والأطوار كلها قُدِّمت 0.10 مع توسيع مداها بالقدر نفسه، فبداياتها
       تبكّر ونهاياتها في مواضعها. سببه أن السقوط خرج من الساعة إلى
       التمرير (أعلاه): فالمشهد يتسلّم ساكناً، وسكونٌ يمتدّ ثانيةً
       كاملة والتمريرُ مقفول يُقرأ تعليقاً لا هدوءاً. الآن يعلو
       البخار بعد 0.31ث من التسليم وتبدأ الرسمةُ تتفكّك بعد 0.76ث.

       الأطوار على الساعة، بالثواني من AUTO_MS=3600:
         البخار     يعلو 0.31ث
         التفكك     من 0.76ث إلى 2.87ث
         الصبّة     من 1.52ث
         طلوع الموج من 1.56ث إلى 2.94ث
         الذوبان    3.24ث → 3.46ث
       فيلتقي اكتمالُ البحر ببداية الذوبان في نقطة واحدة، وتبقى
       الصبّةُ سابقةً للبحر بـ55ms — ضيّقة عمداً وإلا صبّت في الفراغ */
    var steamA = smooth((gs - 0.09) / 0.10) * (1 - smooth((gs - 0.28) / 0.10));

    var dis = smoother(clamp01((gs - 0.20) / 0.72));
    var flood = smoother(clamp01((gs - 0.46) / 0.47));
    ...
    setFade(reduced ? 0 : smoother(clamp01((g - fadeStart) / 0.06)));
  ...
  /* أول لمسةٍ حقيقية من الزائر — لا تمريرَ المتصفح وهو يستعيد
     الموضع. نستمع للسبب لا للأثر: لمسة، عجلة، مفتاح، أو ضغطُ
     مؤشّر (سحب شريط التمرير). وحدث scroll نفسه مستبعَدٌ عمداً
     لأنه بالضبط ما نريد تمييزه عن فعل الزائر. */
  var touched = false;
  (function () {
    var mark = function () { touched = true; };
    var evs = ["touchstart", "wheel", "keydown", "pointerdown"];
    for (var i = 0; i < evs.length; i++) {
      window.addEventListener(evs[i], mark, { passive: true, once: true });
    }
  })();
  ...
  function step(ms) {
    ...
    if (ph === ENTER) {
      ...
      var bTop = band.getBoundingClientRect().top;
      if (bTop < -VP * 0.5) { finish(); return; }

      /* ومن حمّل الصفحة خارجَ الهيرو لا يراها أصلاً: هي مدخلٌ إليه،
         ومن حدّث وهو في المحاصيل أو الحلا يريد بياناتٍ جديدة لا
         عرضاً من أوّله — ولا يصحّ أن يُسحب من موضعه ليشاهده.

         والفحص مشروطٌ بألّا يكون الزائر قد لمس شيئاً بعد، لأن
         المتصفح يستعيد الموضع على دفعات: الصفحة لم يكتمل طولها
         فيضعه أولاً عند رقمٍ أصغر ثم يكمل (قِيس: تحديثٌ عند 3500
         يبدأ عند 1023). ففحصٌ لحظيٌّ واحدٌ يُخدَع، أما هذا فيظلّ
         قائماً حتى أول لمسةٍ حقيقية — فأيّةُ استعادةٍ تتجاوز
         الشريط تُنهي الحكاية قبل أن تبدأ. */
      if (!touched && bTop <= 0) { finish(); return; }
      ...
      var e = raw / TRIG;
      /* ولا تبدأ قبل لمسةٍ من الزائر: تمريرُ الاستعادة ليس تمريرَه،
         ولو تسلّمنا عليه قفلنا الصفحةَ في وجه المتصفح وهو يستعيد */
      if (touched && (e >= 1 || (lastDown && raw >= HOLD_MIN && ms - lastMove > STILL_MS))) {
        ph = AUTO;
  ...
  /* حُمّلت الصفحة والشريطُ فوق الشاشة أصلاً: زائرٌ حدّث وهو في
     المحاصيل أو أبعد. تُختم هنا لا في الحلقة، لأن الحلقة لا تعمل
     والشريطُ خارج النظر — فلولا هذا بقي الكانفس على إطاره الأول
     معتماً، ينكشف له إن صعد إليه. (والاستعادةُ المتدرّجة يتكفّل بها
     شرطُ !touched داخل الحلقة، فالحالتان مسدودتان.) */
  if (!reduced && band.getBoundingClientRect().top <= 0) finish();
  ...
  window.addEventListener("pageshow", function (e) {
    ...
    lastMove = 0;
    lastDown = true;
    /* والرجوع من bfcache يستعيد الموضع كذلك، فيخضع لنفس الشرط:
       من رجع إلى وسط الصفحة لا تُعاد له الحكاية ولا يُسحب */
    touched = false;
    setFade(0);
