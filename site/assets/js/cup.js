...
(function () {
  ...
  var W = 0, H = 0, S = 1, OX = 0, OY = 0, VP = 1, TRACK = 1, LEAD = 220;
  var grain = null;
  ...
  function layout() {
    ...
    /* استباق التسليم: كم بكسل قبل انفكاك التثبيت يكتمل الذوبان.
       الجوال يحتاج أكثر لأن نطّة الإصبع لها اندفاع يكمل بعدها
       فيبتلع الاستباق، والفأرة تقف حيث تقف */
    LEAD = W >= 700 ? 130 : 220;
  ...
  function draw(p, tms) {
    ...
    /* أطوار مرحلة التثبيت تُقاس على لحظة بدء الذوبان لا على لحظة
       انفكاك التثبيت: التسليم صار يستبق الانفكاك بـLEAD (أدناه)،
       فلو بقيت مقيسةً على الانفكاك لبدأ الذوبان والبحرُ لم يغطِّ
       الشاشة بعد، فانكشف القسم ونصفُ المشهد باقٍ. gs تضمن أن يكتمل
       البحر في نفس اللحظة التي يبدأ فيها الذوبان، بلا فجوة ولا
       تداخل، وعلى أي مقاس شاشة */
    var fadeEnd = 1 - LEAD / Math.max(1, TRACK);
    var fadeStart = Math.max(0.1, fadeEnd - 0.045);
    /* مع تقليل الحركة المسارُ صفر فـfadeStart بلا معنى — والطور
       مفروض أصلاً، فتمرّ gs كما هي ولا يذوب المشهد أبداً */
    ...
    /* الأطوار كلها موسّعة: عند التمرير الطبيعي — وهو سريع — كانت
       تمرّ قبل ما تلحقها العين. الأرقام على الجوال ٣٩٠×٨٤٤:
         ...
         السقوط    ١٠٤٩px  (كان ٨٤٤)
         التفكك     ٧٠٦px  (كان ٥٠٢)
         طلوع الموج ٤٠٢px  (كان ٣٦٩)

       والتخميد أبطأ (0.085→0.066) فتلحق النطّة الواحدة من السكرول
       بحركة تمتد بعد ما تقف الإصبع — وهذا علاج التمرير السريع */
    ...
    /* التسليم: بعد ما يكتمل البحر يذوب المشهد كله فينكشف قسم
       ...
       ويخلص قبل انفكاك التثبيت بمسافة (LEAD) لا بـ17px كما كان:
       القسم ينكشف ورأسُه أسفلَ أعلى الشاشة بقليل، فيبدأ من أوله بل
       قبل أوله بشيء — ثم يكمل اندفاعُ الإصبع الباقي فيستقر الرأس
       مكانه. والفراغ فوق الرأس لا يُرى: خلفية الصفحة والقسم كلاهما
       الكريم نفسه */
  ...
  /* ─── التشغيل التلقائي ───
     كان المشهد مربوطاً بالتمرير وحده، فلو وقفت إصبعك وقف معك:
     الكوب معلَّق في منتصف تفكّكه والموجة نصفها. صار للمرحلة
     المثبَّتة ساعةٌ خاصة تنطلق لحظة التثبيت وتمشي وحدها.

     والتمرير لم يُلغَ بل صار أرضيةً لا سقفاً: نأخذ الأكبر من
     الاثنين. فمن وقف رأى الحركة تكمل تلقائياً، ومن مرّر بسرعة
     دفعها للأمام ولم ينتظرها — وفي الحالين لا ترجع للخلف ولا
     تتجمّد نصفَ حركة.

     ويُعاد التشغيل إذا رجع الزائر فوق الشريط، فيراها كاملة ثانيةً
     لا يجدها منتهية.

     والساعة تقف عند بداية الذوبان ولا تتجاوزها: الذوبان تسليمٌ
     هندسي لا حركة — يكشف قسم المحصول المركون خلف المشهد، وهذا
     القسم لا يصل موضعه إلا بالتمرير. فلو أذابته الساعةُ مبكراً
     انكشفت شاشةٌ كريمية فارغة (جُرّب فظهر). فالحركة تلقائية،
     والتسليم يبقى على التمرير حيث ينتمي. */
  var AUTO_MS = 1700;
  var t0 = 0;
  ...
  function frame(ms) {
    ...
    if (target < pin - 0.02) t0 = 0;          // فوق نقطة التثبيت: تصفير
    else if (!t0) t0 = ms;                    // لحظة التثبيت: انطلاق الساعة

    if (t0) {
      var fadeEnd = 1 - LEAD / Math.max(1, TRACK);
      var fadeStart = Math.max(0.1, fadeEnd - 0.045);
      var auto = pin + (1 - pin) * fadeStart * clamp01((ms - t0) / AUTO_MS);
      if (auto > target) target = auto;
    }

    if (pd === null) pd = target;
    pd += (target - pd) * 0.066;
    var lag = LAG_PX / (VP + TRACK);
    if (target - pd > lag) pd = target - lag;
    else if (pd - target > lag) pd = target + lag;
    draw(pd, ms);
    ...
    new IntersectionObserver(
      function (entries) {
        if (entries[0].isIntersecting) {
          ...
          /* الشريط خرج من الشاشة فنوقف الحلقة — لكن لا نتركها واقفة
             على إطارٍ ناقص: لو خرج والتخميد ما لحق تجمّدت الشفافية
             في منتصفها، فيرجع الزائر لاحقاً على مشهدٍ نصف ذائب.
             نرسم إطاراً أخيراً بالموضع الحقيقي قبل الوقوف */
          running = false;
          pd = progress();
          draw(pd, 0);
        }
