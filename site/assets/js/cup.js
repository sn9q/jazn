...
(function () {
  ...
  /* ─── التشغيل التلقائي ───
     ...
     ولا تنطلق الساعة إلا حين يبلغ الشريط أعلى الشاشة فيملأها.
     جُرّب انطلاقها عند ظهور نصفه فسقط الطقم تحت الطيّة: المشهد
     مركزُه وسطُ الشريط، فإن كان نصفه تحت الشاشة كان مركزه تحتها،
     فلا يُرى السقوط ولا الاستقرار — ويصل الزائرُ والكوبُ مستقرٌّ
     كأنه معلَّق. وقبل ذلك يُرسم الإطار الأول ثابتاً، فلا فراغ. */
  var AUTO_MS = 3400;
  var t0 = 0;
  ...
  function frame(ms) {
    if (!running) return;
    if (!t0) {
      if (band.getBoundingClientRect().top > 6) {
        draw(0, ms);
    ...
    new IntersectionObserver(
      function (entries) {
        if (entries[0].isIntersecting) {
          ...
          /* خرج الشريط: نوقف الحلقة ونصفّر الساعة، فمن رجع إليه
             رآه من أوله لا منتهياً */
          running = false;
          t0 = 0;
          pd = null;
          draw(0, 0);
