...
(function () {
  ...
  function draw(p, tms) {
    ...
    /* p صار خطّاً زمنياً من 0 إلى 1 لا نسبةَ تمرير: الشريط شاشة
       واحدة بلا مسار، والمشهد يمشي بساعته. فالسقوط يأخذ أول 34%
    ...
    var visTop = Math.max(0, rb.top);
    var visBot = Math.min(VP, rb.bottom);
    OY = (visTop + visBot) / 2 - rb.top - (VH * S) / 2;
    ...
    var topScene = ((visTop - rb.top) - OY) / S;
  ...
  /* ─── التشغيل التلقائي ───
     المشهد كان مربوطاً بالتمرير: نسبةُ تقدّمه هي نسبةُ ما قطعتَه
     من مسارٍ طوله شاشتان ونصف. فلو وقفت وقف، ولو أسرعت أسرع، ولا
     ينكشف قسم المحصول إلا بعد أن تقطع المسار كلَّه.

     صار خطاً زمنياً يمشي وحده: يبدأ أول ما يدخل الشريط الشاشةَ،
     ويمضي 2.6 ثانية من السقوط إلى الموج إلى الذوبان، فينكشف القسم
     في مكانه بلا أن تمرّر. والشريط انكمش إلى شاشة واحدة لأن المسار
     الطويل كان يوجد الزمن، والزمن الآن من الساعة.

     وتنطلق عند أول بكسل يظهر من الشريط، لا عند ثلثه: العتبة كانت
     تؤخّرها، وتترك الكانفس قبلها على إطارٍ رُسم والشريطُ في موضعٍ
     آخر — فيظهر الصحن ثابتاً في مكانٍ لا يخصّه. الآن تُرسم من أول
     لحظةٍ بموضعها الصحيح.

     وتُعرض مرةً واحدة: من نزل ثم رجع وجدها منتهية لا مستأنفة —
  ...
  function frame(ms) {
    ...
    if (done) return;
    if (!t0) t0 = ms;
    var target = clamp01((ms - t0) / AUTO_MS);
    if (target >= 1) done = true;
    if (pd === null) pd = target;
    /* تخميدٌ خفيف: الساعة منتظمة، وهذا يذيب حوافّ البداية والنهاية */
    pd += (target - pd) * 0.22;
    draw(pd, ms);
    ...
    new IntersectionObserver(
      function (entries) {
        if (entries[0].isIntersecting) {
          if (done) { draw(1, 0); return; }   // انتهت: يبقى المشهد ذائباً
          if (!running) {
            running = true;
            window.requestAnimationFrame(frame);
          }
