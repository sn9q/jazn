...
(function () {
  ...
  function draw(p, tms) {
    ...
    /* ── إيقاع التسليم: للجوال إيقاعه وللديسكتوب إيقاعه ──
       hnd يقيس على ارتفاع الشاشة، وعلى جوال طويل ضيّق (844 مقابل
       شريط 437) تطلع مرحلة التسليم ضعف مرحلة الاقتراب، فيصير التفكك
       يتمطّط: ربع الصفحة التالية طالع والبحر لسّه 3%. فنافذة أضيق
       على الجوال وأوسع شوي على الديسكتوب — المشهد هناك أكبر
       والصبّة تحتاج مدى تُقرأ فيه. الحد 700px هو نفسه حدّ الـCSS.
       المعيار في الحالتين: البحر يكتمل قبل ما يطلع ربع الصفحة
       التالية (hnd = 0.25)، فالكريم يستلم قبل ما تبان القائمة. */
    var narrow = W < 700;
    var DIS_W = narrow ? 0.27 : 0.29;
    var FLD_S = narrow ? 0.120 : 0.130;
    var FLD_W = narrow ? 0.155 : 0.165;

    var dis = smooth(clamp01(hnd / DIS_W));
    var flood = smooth(clamp01((hnd - FLD_S) / FLD_W));
    ...
    if (cRaw > 0.001) {
      ...
      /* البحر يطلع من الصبّة فلا يصح يسبقها. مع ضيق نافذة التسليم
         صارت الصبّة تبدأ متأخرة داخلها، فقدّمناها في عمر الجسم
         (0.16→0.10) وسرّعنا وصولها (0.5→0.42) بدل ما نوسّع النافذة:
         تبدأ عند hnd≈0.084 قبل البحر (0.120) بـ30px من التمرير على
         الجوال و36px على الديسكتوب — فجوة ضيقة عمداً وإلا صبّت
         الخيوط في الفراغ. */
