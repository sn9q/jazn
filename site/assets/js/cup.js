/* ============================================================
   ...
   إلى 1 عند انفكاك التثبيت — وهي نفسها لحظة ظهور رأس المحصول
   أسفل الشاشة. فالبحر يكتمل قبلها بقليل، ويستلم القسمُ كريماً
   على كريم بلا خيط يفصلهما.
  ...
  function draw(p, tms) {
    ...
    /* ── المسار مرحلتان تفصلهما لحظة التثبيت ──
       ...
       وهي حدث بصري حقيقي (المشهد يقف والصفحة تستمر) فنعلّق عليه:
         ...
         f = 0→1 مرحلة الدخول: الطقم يسقط، ويستقر عند 1 بالضبط
             أي في اللحظة اللي يقف فيها المشهد — فما يستقر وهو
             لسّه يتحرك مع الصفحة
         g = 0→1 مرحلة التثبيت: بخار، ثم تفكك وصبّ وبحر
    ...
    var sRaw = clamp01((f - 0.06) / 0.62);
    var sFall = sRaw * sRaw;
    var sDip = Math.sin(clamp01((f - 0.68) / 0.20) * Math.PI);
    var cRaw = clamp01((f - 0.06) / 0.82);
    var cFall = cRaw * cRaw;
    var land = clamp01((f - 0.88) / 0.12);
    var bounce = Math.sin(Math.min(1, land) * Math.PI);
    var steamA = smooth((g - 0.02) / 0.13) * (1 - smooth((g - 0.26) / 0.10));

    /* نَفَس الاستقرار قصير (g حتى 0.16، أي ١٢٢px على الجوال) ثم
       يبدأ التفكك ويأخذ سبعة أعشار مرحلة التثبيت — ٥٣٢px على
       الجوال و٧٢٤px على الديسكتوب، فتقرأ العين كل طور منه.

       والبحر يطلع في 0.55 من المرحلة (٤١٨px على الجوال، ٥٦٩px على
       الديسكتوب) ويكتمل عند 0.97 — أي عند انفكاك التثبيت نفسه لا
       قبله: ذيل الكريم الكامل كان ١٠٦px من شاشة بيضاء لا يصير
       فيها شي، فقُصّ لـ٢٣px تكفي ليلحق التخميدُ آخرَ الموجة */
    var dis = smooth(clamp01((g - 0.16) / 0.66));
    var flood = smooth(clamp01((g - 0.42) / 0.55));
    ...
    if (cRaw > 0.001) {
      ...
      /* البحر يطلع من الصبّة فلا يصح يسبقها: الصبّة تبدأ عند
         g≈0.365 قبل البحر (0.42) بـ42px من التمرير على الجوال
         و57px على الديسكتوب — فجوة ضيقة عمداً وإلا صبّت الخيوط
         في الفراغ */
